# Site de Spectacles - PHP Pur avec JWT et 2FA

Application web complète de gestion de spectacles développée en PHP pur avec architecture MVC, système d'authentification JWT, refresh tokens, authentification à deux facteurs (2FA) et middleware sous forme d'attributs PHP 8.

## Fonctionnalités

### Pages Publiques
- **Page d'accueil** : Message de bienvenue avec menu de navigation
- **Liste des spectacles** : Affichage de tous les spectacles disponibles
- **Fiche spectacle** : Détails complets d'un spectacle

### Utilisateurs Authentifiés (ROLE_USER)
- **Réservation de places** : Réserver des billets pour les spectacles
- **Page de profil** : Voir toutes ses réservations
- **Authentification à deux facteurs (2FA)** : Sécuriser son compte avec Email, SMS ou TOTP

### Administrateurs (ROLE_ADMIN)
- **Ajouter des spectacles** : Créer de nouveaux spectacles

## Architecture Technique

### Structure du Projet
\`\`\`
/public
  index.php          # Point d'entrée unique
  router.php         # Routeur pour serveur PHP intégré
  .htaccess          # Réécriture d'URL
  /css
    style.css        # Styles globaux
/src
  /Core
    Router.php       # Routeur avec support des paramètres dynamiques
    Request.php      # Encapsulation des requêtes HTTP
  /Controller
    HomeController.php
    AuthController.php
    SpectacleController.php
    BookingController.php
    ProfileController.php
    AdminController.php
    TwoFactorController.php  # Nouveau : Gestion 2FA
  /Service
    JWTService.php           # Gestion des JWT (access + refresh tokens)
    AuthService.php          # Authentification
    SpectacleService.php     # CRUD spectacles
    BookingService.php       # Gestion réservations
    TwoFactorService.php     # Nouveau : Gestion 2FA (Email, SMS, TOTP)
  /Middleware
    IsGranted.php    # Attribut PHP 8 pour la sécurité
  /View
    *.php            # Templates HTML
    2fa_*.php        # Nouveau : Pages 2FA
/config
  routes.php         # Définition des routes
/vendor
  autoload.php       # Autoloader PSR-4
\`\`\`

### Technologies Utilisées
- **PHP 8.0+** : Attributs, typage strict, namespaces
- **JWT (JSON Web Tokens)** : Authentification stateless
- **Refresh Tokens** : Prolongation automatique de session
- **2FA (Two-Factor Authentication)** : Email, SMS, TOTP (RFC 6238)
- **Attributs PHP 8** : Middleware déclaratif avec `#[IsGranted]`
- **Architecture MVC** : Séparation des responsabilités
- **PSR-4 Autoloading** : Chargement automatique des classes
- **Routeur personnalisé** : Support des URLs dynamiques

## Système de Sécurité

### JWT (JSON Web Tokens)
- **Access Token** : Durée de vie courte (15 minutes)
- **Refresh Token** : Durée de vie longue (7 jours)
- **Signature HMAC-SHA256** : Garantit l'intégrité du token
- **Cookie HTTP-only** : Protection contre les attaques XSS
- **SameSite Strict** : Protection contre les attaques CSRF

### Authentification à Deux Facteurs (2FA)

Le système supporte trois méthodes de vérification en deux étapes :

#### 1. Email (Code à 6 chiffres)
- Un code est envoyé par email après la connexion
- Durée de validité : 5 minutes
- **Mode développement** : Le code est affiché dans la session (pas d'envoi réel)

#### 2. SMS (Code à 6 chiffres)
- Un code est envoyé par SMS après la connexion
- Durée de validité : 5 minutes
- **Mode développement** : Le code est affiché dans la session (pas d'envoi réel)

#### 3. TOTP (Time-based One-Time Password)
- Compatible avec Google Authenticator, Microsoft Authenticator, etc.
- Génération de QR code pour configuration facile
- Algorithme RFC 6238 implémenté en PHP pur
- Codes valides pendant 30 secondes
- **Recommandé** : La méthode la plus sécurisée

### Flux d'Authentification avec 2FA

1. **Connexion initiale** : L'utilisateur entre son identifiant et mot de passe
2. **Vérification 2FA** : Si activé, l'utilisateur est redirigé vers la page de vérification
3. **Validation du code** : L'utilisateur entre le code reçu (Email/SMS) ou généré (TOTP)
4. **Accès accordé** : Si le code est valide, les tokens JWT sont créés

### Configuration du 2FA

Les utilisateurs peuvent configurer le 2FA depuis leur profil :
- Accéder à "Mon Profil" → "Gérer l'authentification à deux facteurs"
- Choisir une méthode (Email, SMS ou TOTP)
- Pour TOTP : Scanner le QR code avec une application d'authentification
- Vérifier avec un code de test
- Le 2FA est activé et sera demandé à chaque connexion

### Middleware avec Attributs PHP 8
\`\`\`php
// Protège une méthode - nécessite d'être connecté
#[IsGranted('ROLE_USER')]
public function book() { ... }

// Protège une méthode - nécessite le rôle admin
#[IsGranted('ROLE_ADMIN')]
public function create() { ... }
\`\`\`

### Validation des Tokens
1. Vérification de l'existence du cookie
2. Validation de la signature (non modifié)
3. Vérification de l'expiration
4. Vérification du rôle si spécifié

## Installation et Lancement

### Prérequis
- PHP 8.0 ou supérieur
- Serveur web (Apache, Nginx) ou PHP built-in server
- Module Apache `mod_rewrite` activé (si Apache)

### Installation

1. **Clonez ou téléchargez le projet**
\`\`\`bash
git clone <url-du-projet>
cd spectacles-php
\`\`\`

2. **Vérifiez la version de PHP**
\`\`\`bash
php -v
# Doit afficher PHP 8.0 ou supérieur
\`\`\`

### Lancement avec le serveur PHP intégré (RECOMMANDÉ)

**IMPORTANT : Utilisez cette commande exacte pour éviter l'erreur "Not Found"**

\`\`\`bash
# Depuis la racine du projet
php -S localhost:8000 -t public public/router.php
\`\`\`

**Explication :**
- `-S localhost:8000` : Lance le serveur sur le port 8000
- `-t public` : Définit le dossier public comme racine web
- `public/router.php` : Utilise le routeur personnalisé (nécessaire car le serveur PHP intégré ne lit pas .htaccess)

**Ouvrez ensuite votre navigateur :**
\`\`\`
http://localhost:8000
\`\`\`

**Autres ports disponibles si 8000 est occupé :**
\`\`\`bash
php -S localhost:3000 -t public public/router.php
php -S localhost:8080 -t public public/router.php
\`\`\`

### Lancement avec XAMPP/WAMP/MAMP

1. **Copiez le projet dans le dossier web**
   - XAMPP : `C:\xampp\htdocs\spectacles-php`
   - WAMP : `C:\wamp64\www\spectacles-php`
   - MAMP : `/Applications/MAMP/htdocs/spectacles-php`

2. **Accédez au projet**
\`\`\`
http://localhost/spectacles-php/public
\`\`\`

3. **Configuration Apache** (si nécessaire)
   - Assurez-vous que `mod_rewrite` est activé
   - Le fichier `.htaccess` doit être présent dans `/public`

## Comptes de Test

### Administrateur
- **Identifiant** : `admin`
- **Mot de passe** : `admin123`
- **Permissions** : Toutes (créer spectacles, réserver, etc.)

### Utilisateur Standard
- **Identifiant** : `user`
- **Mot de passe** : `user123`
- **Permissions** : Réserver des places, voir son profil

## Utilisation

### 1. Connexion
- Accédez à `/login`
- Utilisez un des comptes de test
- Un JWT est créé et stocké dans un cookie sécurisé

### 2. Configuration du 2FA (Nouveau)
- Connectez-vous avec un compte
- Allez sur "Mon Profil"
- Cliquez sur "Gérer l'authentification à deux facteurs"
- Choisissez une méthode :
  - **Email** : Recevez un code par email (simulé en dev)
  - **SMS** : Recevez un code par SMS (simulé en dev)
  - **TOTP** : Scannez le QR code avec Google Authenticator

### 3. Connexion avec 2FA
- Après avoir activé le 2FA, à chaque connexion :
  1. Entrez votre identifiant et mot de passe
  2. Vous êtes redirigé vers la page de vérification 2FA
  3. Entrez le code reçu (Email/SMS) ou généré (TOTP)
  4. Accès accordé si le code est valide

### 4. Parcourir les Spectacles
- Cliquez sur "Spectacles" dans le menu
- Consultez les détails d'un spectacle

### 5. Réserver une Place (Utilisateur connecté)
- Sur la fiche d'un spectacle, cliquez sur "Réserver une place"
- La réservation est ajoutée à votre profil

### 6. Voir ses Réservations
- Cliquez sur "Mon Profil"
- Toutes vos réservations sont listées

### 7. Ajouter un Spectacle (Admin uniquement)
- Connectez-vous avec le compte admin
- Cliquez sur "Ajouter un spectacle"
- Remplissez le formulaire

## Refresh Token - Prolongation de Session

Le système utilise des **refresh tokens** pour prolonger automatiquement la session sans redemander les identifiants.

### Fonctionnement
1. À la connexion, deux tokens sont créés :
   - **Access token** (15 min) : stocké dans un cookie
   - **Refresh token** (7 jours) : stocké en session PHP

2. Quand l'access token expire, le client peut appeler `/refresh-token` avec le refresh token pour obtenir un nouvel access token

3. Cela permet de rester connecté pendant 7 jours sans redemander le mot de passe

### Exemple d'utilisation (JavaScript)
\`\`\`javascript
// Appel automatique avant expiration de l'access token
fetch('/refresh-token', {
  method: 'POST',
  credentials: 'include'
})
.then(response => response.json())
.then(data => {
  console.log('Token renouvelé');
});
\`\`\`

## Implémentation TOTP (RFC 6238)

Le système implémente l'algorithme TOTP en PHP pur sans dépendances externes :

### Caractéristiques
- **Algorithme** : HMAC-SHA1 (standard RFC 6238)
- **Période** : 30 secondes
- **Longueur du code** : 6 chiffres
- **Encodage** : Base32 pour le secret
- **Tolérance** : ±1 période (compense le décalage temporel)

### Compatibilité
- Google Authenticator
- Microsoft Authenticator
- TOTP Authenticator
- Authy
- Toute application compatible RFC 6238

## Points Techniques Importants

### Routeur Dynamique
Le routeur supporte les paramètres dans les URLs :
\`\`\`php
$router->add('GET', '/spectacles/{id}', 'SpectacleController', 'show');
// Correspond à : /spectacles/1, /spectacles/2, etc.
\`\`\`

### Attributs PHP 8 pour la Sécurité
Les attributs permettent une sécurité déclarative :
\`\`\`php
#[IsGranted('ROLE_ADMIN')]
public function create() {
    // Cette méthode n'est accessible qu'aux admins
}
\`\`\`

### Autoloader PSR-4
Les classes sont chargées automatiquement selon leur namespace :
\`\`\`php
namespace App\Controller;
// Fichier : src/Controller/HomeController.php
\`\`\`

### Stockage des Données
- **Utilisateurs** : Tableau PHP statique dans AuthService
- **Spectacles** : Session PHP ($_SESSION['spectacles'])
- **Réservations** : Session PHP ($_SESSION['bookings'])
- **Préférences 2FA** : Session PHP ($_SESSION['user_2fa_preferences'])

En production, utilisez une vraie base de données (MySQL, PostgreSQL).

## Sécurité en Production

Pour un déploiement en production, pensez à :

1. **Changer la clé secrète JWT** dans `JWTService.php`
2. **Activer HTTPS** et mettre `secure => true` dans les cookies
3. **Utiliser une vraie base de données** (MySQL, PostgreSQL)
4. **Hasher les mots de passe** avec `password_hash()` et `password_verify()`
5. **Implémenter l'envoi réel d'emails** (SendGrid, Mailgun, etc.)
6. **Implémenter l'envoi réel de SMS** (Twilio, Vonage, etc.)
7. **Valider et assainir toutes les entrées utilisateur**
8. **Ajouter des logs** pour tracer les actions sensibles
9. **Implémenter un rate limiting** contre les attaques par force brute
10. **Utiliser des variables d'environnement** pour les secrets
11. **Stocker les secrets TOTP de manière sécurisée** (chiffrement en base de données)

## Dépannage

### Erreur "Not Found - The requested resource / was not found"
**Cause :** Le serveur PHP intégré ne lit pas le fichier `.htaccess`

**Solution :** Utilisez la commande complète avec le routeur :
\`\`\`bash
php -S localhost:8000 -t public public/router.php
\`\`\`

### Erreur 404 sur toutes les pages (Apache/XAMPP)
- Vérifiez que `mod_rewrite` est activé (Apache)
- Vérifiez que le fichier `.htaccess` est présent dans `/public`

### Les cookies ne sont pas stockés
- Vérifiez que la session PHP est démarrée (`session_start()`)
- Vérifiez les paramètres de cookies dans le navigateur

### Erreur "Class not found"
- Vérifiez que l'autoloader est bien chargé
- Vérifiez les namespaces des classes

### Le refresh token ne fonctionne pas
- Vérifiez que la session PHP est active
- Vérifiez que le refresh token est bien stocké en session

### Le code 2FA n'est pas accepté (TOTP)
- Vérifiez que l'heure du serveur est correcte (TOTP dépend du temps)
- Attendez le prochain code (30 secondes)
- Vérifiez que le secret a été correctement scanné

### Les réservations ne s'affichent pas
- Vérifiez que les sessions PHP sont actives
- Les données sont stockées en session et seront perdues au redémarrage du serveur

## Licence

Projet éducatif - Libre d'utilisation

## Auteur

Développé comme exercice d'apprentissage PHP avec JWT, 2FA et architecture MVC
